...
impl World {
    ...
    fn exec_build(&mut self, at: Pos, class: BuildingClass) {
        ...
        let cost = class.get_build_property().unwrap().item_cost;
        self.unitmap.get_mut(at).unwrap().inventory.reduce(cost);
    }

    fn exec_work(&mut self, at: Pos) {
        let mut tmp_opt: Option<_> = None;
        mem::swap(&mut tmp_opt, self.buildingmap.get_mut_raw(at));
        tmp_opt.iter_mut().for_each(|b| b.work(self, at));
    ...
    fn exec_drop_item(&mut self, at: Pos, i: usize, opt_dir: Option<Direction>) {
        ...
        self.itemmap.get_mut(droppos).push(item);
    }

    fn exec_take_item(&mut self, at: Pos, i: usize) {
        let item = self.itemmap.get_mut(at).get_item_vec().remove(i);
        self.unitmap
            .get_mut(at)
            .unwrap()
            .inventory
    ...
    fn exec_exec_item(&mut self, i: usize, at: Pos) {
        let item = self.unitmap.get_mut(at).unwrap().inventory.remove(i);
        item.exec(at, self);
    }
