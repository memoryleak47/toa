    ...
    pub fn animate(&mut self, c: &Command) {
        match c {
            Command::UnitCommand {
                ...
                self.animationmap
                    .set(*pos, Some(Animation::new(AnimationKind::Burn)));
            }
            Command::UnitCommand {
                command: UnitCommand::Attack(a, b),
                pos,
            } => {
                let unit = self.world.unitmap.get(*pos).unwrap();
                let rel_tiles = a.map(|i| unit.inventory.get(i)).aim(*b);
                for t in rel_tiles {
                    let p = if let Some(x) = pos.map(|p| p + t) {
                        x
                    } else {
                        continue;
                    };
                    self.animationmap
                        .set(p, Some(Animation::new(AnimationKind::Damage)));
                }
            }
            _ => {}
        }
