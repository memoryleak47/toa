...
impl UnitCommand {
    ...
    pub fn get_stamina_cost(&self, pos: Pos, w: &World) -> u32 {
        match self {
            ...
            UnitCommand::Build(class) => class.get_build_property().unwrap().stamina_cost,
...
pub fn stamina_cost_at(pos: Pos, w: &World) -> u32 {
    w.buildingmap
        .get(pos)
        .and_then(|x| x.get_class().reduces_walk_stamina())
        .unwrap_or_else(|| w.terrainmap.get(pos).get_stamina_cost())
}
