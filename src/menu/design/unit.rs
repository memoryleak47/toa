...
impl App {
    pub(super) fn build_unit_pane(&self, offset: Vec2f) -> Vec<Widget> {
        let u = if let Some(x) = self.world.unitmap.get(self.cursor) {
            x
        } else {
            return Vec::new();
        };
        ...
        widgets.push(Widget {
            ...
            size: ws * 0.025,
            draw_type: if matches!(self.menu_state, MenuState::Attack(_)) {
                Color::rgb(200, 0, 0)
            } else {
                Color::rgb(100, 0, 0)
            }
        ...
        widgets.push(Widget {
            ...
            size: ws * 0.025,
            draw_type: if matches!(self.menu_state, MenuState::ExecItem) {
                Color::rgb(0, 200, 0)
            } else {
                Color::rgb(0, 100, 0)
            }
        ...
        widgets.push(Widget {
            ...
            size: ws * 0.025,
            draw_type: if matches!(self.menu_state, MenuState::DropItem(_)) {
                Color::rgb(0, 0, 200)
            } else {
                Color::rgb(0, 0, 100)
            }
            .into(),
            on_click: vec![MenuCommand::StateChange(
                MenuState::DropItem(HashSet::new()),
            )],
    ...
    fn build_unit_inv_pane(&self, u: &Unit, offset: Vec2f) -> Vec<Widget> {
        ...
        let extra_draw = |i: usize| -> Option<DrawType> {
            match self.menu_state {
                MenuState::Attack(Some(j)) if i == j => Some(Color::rgba(255, 0, 0, 20).into()),
        ...
        let on_click = |i| match self.menu_state {
            ...
            MenuState::DropItem(ref current_indices) => {
                ...
                vec![MenuCommand::StateChange(MenuState::DropItem(
                    current_indices,
                ))]
            }
            _ => Vec::new(),
