...
impl App {
	...
	fn render_buildingmap(&mut self) {
		for p in Pos::iter_all() {
			if let Some(ref building) = self.world.buildingmap.get(p) {
				...
				self.render_gameobject(p, &building);
			}
		}
	}
	...
	fn render_unitmap(&mut self) {
		for p in Pos::iter_all() {
			if let Some(u) = self.world.unitmap.get(p) {
				let u: Unit = u.clone(); // TODO
				self.render_gameobject(p, &u);
				self.render_gameobject(p, &Cloth(u.owner));

				if let Some(ref main_item) = self.world.unitmap.get(p).unwrap().main_item {
					let main_item = main_item.clone(); // TODO
					self.render_gameobject(p, &main_item);
				}
			}
		}
	}
		...
		if let Some(tiles) = opt_tiles {
			for rel in tiles.into_iter() {
				if let Some(p) = cursor.map(|x| x + rel) {
					self.render_gameobject(p, &Marker::Combat);
				}
			}
		}
	}
	...
	fn render_gameobject<GO: GameObject>(&mut self, p: Pos, go: &GO) {
